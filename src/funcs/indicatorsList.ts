...
export function indicatorsList(
  ...
  Result<
    components.GetAvailableIndicatorsResponse,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const path = pathToFunc("/v1/assets/futures/indicators")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetAvailableIndicatorsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
