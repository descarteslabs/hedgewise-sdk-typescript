...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetSupplyRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/supply/{symbol}")(pathParams);

  const query = encodeFormQuery({
    "country_code": payload.country_code,
    "end_date": payload.end_date,
    "get_feature_contributions": payload.get_feature_contributions,
    "model": payload.model,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetSupplyResponse$inboundSchema),
