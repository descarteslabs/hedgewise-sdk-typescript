 ...
 */
export function performanceMetricsGet(
  client: HedgewiseCore,
  request: operations.GetPerformanceMetricsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetPerformanceMetricsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetPerformanceMetricsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/models/performance/{symbol}")(pathParams);
  ...
  const query = encodeFormQuery({
    ...
    "threshold_on_forecast": payload.threshold_on_forecast,
    "use_run_date": payload.use_run_date,
  });
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetPerformanceMetricsResponse$inboundSchema),
