 ...
 */
export function featuresGetHistoricalValues(
  client: HedgewiseCore,
  request: operations.GetFeaturesHistoricalValuesRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetFeatureHistoricalResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetFeaturesHistoricalValuesRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    feature_code: encodeSimple("feature_code", payload.feature_code, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/features/historical/{feature_code}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.GetFeatureHistoricalResponse$inboundSchema),
