 ...
 */
export function futuresGetHedgeIndicator(
  client: HedgewiseCore,
  request: operations.GetHedgeIndicatorRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetHedgeIndicatorResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetHedgeIndicatorRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/assets/futures/indicators/hedge/{symbol}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetHedgeIndicatorResponse$inboundSchema),
