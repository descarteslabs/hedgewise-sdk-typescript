 ...
 */
export function strategiesGetStrategyForecast(
  client: HedgewiseCore,
  request: operations.GetStrategyForecastRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    components.GetStrategyForecastResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetStrategyForecastRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    strategy_id: encodeSimple("strategy_id", payload.strategy_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/v1/strategies/{strategy_id}/forecast")(pathParams);
  ...
  const context = {
    ...
    operationID: "get_strategy_forecast",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.GetStrategyForecastResponse$inboundSchema),
